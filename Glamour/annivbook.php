...
session_start();
    ...
    $query = "SELECT venuename FROM venues WHERE venuetype = 'Anniversary' ";
    $queryres = mysqli_query($connection, $query);

    $userresult = mysqli_query($connection, "SELECT * FROM users WHERE user_id = $id");
    $userrow = mysqli_fetch_assoc($userresult);

    $query2 = "SELECT theme_name FROM theme WHERE theme_type = 'Anniversary' ";
    $themeres = mysqli_query($connection, $query2);

    //unique id
    $checkid = "SELECT id FROM anniversary ORDER BY id DESC LIMIT 1"; // select only id column
        ...
        $uid = $row['id'];
        $get_numbers = str_replace("ANNIV-", "", $uid);
        $id_increase = $get_numbers+1;
        $get_string = str_pad($id_increase, 4, 0, STR_PAD_LEFT);
        $newid = "ANNIV-".$get_string; 
    ...
    if(isset($_POST['submit']))
    {
        ...
        if(strtotime($bookdate) < time()) {
            ...
            //
                ...
                $venueQuery = "SELECT $priceColumn FROM venues WHERE venuename = '$venue' AND venuetype = 'Anniversary' ";
                ...
                $styleQuery = "SELECT price FROM prices WHERE price_name = '$style' AND price_guestnum = '$guests' AND price_event = 'Anniversary' ";
                ...
                // Retrieve the price based on the services options
                    ...
                    $sql_booking = "INSERT INTO booking (user_id, booking_id, firstname, lastname, phone, address, emailadd, bookdate, weektype, event, amount, downpayment, venueprice, cuisineprice, styleprice, serviceprice, status, coordinator_1, coordinator_2, coordinator_3, coordinator_4) VALUES ('$id', '$newid', '$firstname', '$lastname', '$phone', '$address', '$emailadd', '$bookdate', '$weektype', 'Anniversary', '$finalPrice', '$downPayment', '$venuePrice', '$cuisinePrice', '$stylePrice', '$totalPrice', 'pending', '$coordinator1', '$coordinator2', '$coordinator3', '$coordinator4' )";
                    $result = mysqli_query($connection, $sql_booking);

                $sql_booking2 = "INSERT INTO anniversary (user_id, booking_id, venue, guest_number, cuisine, style, theme_design, extra_services, other_preferences) VALUES ('$id', '$newid', '$venue', '$guests', '$cuisine', '$style', '$theme', '$services', '$message')";
                ...
                if($result && $result2)
                {
                    ...
                    window.location.href='annivbook.php';
                    </script>
                    <?php
                } else {
...
<head>
    ...
    <title>Book an Anniversary | Glamour</title>
...
<body>
        ...
        <ul>
            ...
            <li>
                ...
                <ul class="dropdown">
                    ...
                    <li><a href="anniversary-logged.php" class="active" style="font-size: .7em">Anniversary</a></li>
    ...
    <div class="booking-form" id="booking-form">
        <div class="col1">
            ...
            <div class="bookpic">
                <img src="CSS/Pictures/anniversary/anniversarybookdisplay.png" style="width: 100%;">
            </div>
        </div>
        <div class="col2">
        <h2>Venue</h2>
