...
session_start();
    ...
    $query = "SELECT venuename FROM venues WHERE venuetype = 'Christening' ";
    $queryres = mysqli_query($connection, $query);

    $query2 = "SELECT theme_name FROM theme WHERE theme_type = 'Christening' ";
    $themeres = mysqli_query($connection, $query2);


    //unique id
    $checkid = "SELECT id FROM christening ORDER BY id DESC LIMIT 1"; // select only id column
        ...
        $uid = $row['id'];
        $get_numbers = str_replace("CHRIS-", "", $uid);
        $id_increase = $get_numbers+1;
        $get_string = str_pad($id_increase, 4, 0, STR_PAD_LEFT);
        $newid = "CHRIS-".$get_string; 
        ...
        if(strtotime($bookdate) < time()) {
            ...
            //
                ...
                if(empty($_POST['guests']) || empty( $_POST['gender']) || empty($_POST['venue'])  || empty( $_POST['cuisine'])  || empty( $_POST['style']) || empty( $_POST['theme'])  || empty( $_POST['checkbox']))
                {
                    ...
                    $venueQuery = "SELECT $priceColumn FROM venues WHERE venuename = '$venue' AND venuetype = 'Christening' ";
                    ...
                    $styleQuery = "SELECT price FROM prices WHERE price_name = '$style' AND price_guestnum = '$guests' AND price_event = 'Christening' ";
                    ...
                    $sql_booking = "INSERT INTO booking (user_id, booking_id, firstname, lastname, phone, address, emailadd, bookdate, weektype, event, amount, downpayment, venueprice, cuisineprice, styleprice, serviceprice, status, coordinator_1, coordinator_2, coordinator_3, coordinator_4) VALUES ('$id', '$newid', '$firstname', '$lastname', '$phone', '$address', '$emailadd', '$bookdate', '$weektype', 'Christening', '$finalPrice', '$downPayment', '$venuePrice', '$cuisinePrice', '$stylePrice', '$totalPrice', 'pending', '$coordinator1', '$coordinator2', '$coordinator3', '$coordinator4' )";
                    $result = mysqli_query($connection, $sql_booking);

                    $sql_booking2 = "INSERT INTO christening (user_id, booking_id, celeb_gender, venue, guest_number, cuisine, style, theme_design, extra_services, other_preferences) VALUES ('$id', '$newid', '$gender', '$venue', '$guests', '$cuisine', '$style', '$theme', '$services', '$message')";
                ...
                if($result && $result2)
                {
                    ...
                    window.location.href='christeningbook.php';
                    </script>
                    <?php
                } else {
...
<head>
    ...
    <title>Book a Christening | Glamour</title>
...
<body>
    ...
    <div class="booking-form" id="booking-form">
        <div class="col1">
            ...
            <div class="bookpic">
                <img src="CSS/Pictures/christening/christeningbookdisplay.png" style="width: 100%;">
            </div>
        </div>
        <div class="col2">
            <h2>Celebrant's Gender</h2>
             <select id="gender" name="gender">
                 <option value="" disabled selected >Select baby's gender</option>
