...
session_start();
    ...
    $query = "SELECT venuename FROM venues WHERE venuetype = 'Corporate' ";
    $queryres = mysqli_query($connection, $query);

    $query2 = "SELECT theme_name FROM theme WHERE theme_type = 'Corporate' ";
    $themeres = mysqli_query($connection, $query2);

    //unique id
    $checkid = "SELECT id FROM corporate ORDER BY id DESC LIMIT 1"; // select only id column
        ...
        $uid = $row['id'];
        $get_numbers = str_replace("CORP-", "", $uid);
        $id_increase = $get_numbers+1;
        $get_string = str_pad($id_increase, 4, 0, STR_PAD_LEFT);
        $newid = "CORP-".$get_string; 
    ...
    if(isset($_POST['submit']))
    {
        ...
        if(strtotime($bookdate) < time()) {
            ...
            //
                ...
                if(empty($_POST['guests']) || empty($_POST['venue'])  || empty( $_POST['cuisine'])  || empty( $_POST['theme'])  || empty( $_POST['checkbox']))
                {
                    ...
                    $venueQuery = "SELECT $priceColumn FROM venues WHERE venuename = '$venue' AND venuetype = 'Corporate' ";
                    ...
                    $styleQuery = "SELECT price FROM prices WHERE price_name = '$style' AND price_guestnum = '$guests' AND price_event = 'Corporate' ";
                    ...
                    $sql_booking = "INSERT INTO booking (user_id, booking_id, firstname, lastname, phone, address, emailadd, bookdate, weektype, event, amount, downpayment, venueprice, cuisineprice, styleprice, serviceprice, status, coordinator_1, coordinator_2, coordinator_3, coordinator_4) VALUES ('$id', '$newid', '$firstname', '$lastname', '$phone', '$address', '$emailadd', '$bookdate', '$weektype', 'Corporate', '$finalPrice', '$downPayment', '$venuePrice', '$cuisinePrice', '$stylePrice', '$totalPrice', 'pending', '$coordinator1', '$coordinator2', '$coordinator3', '$coordinator4' )";
                    $result = mysqli_query($connection, $sql_booking);

                    $sql_booking2 = "INSERT INTO corporate (user_id, booking_id, venue, guest_number, cuisine, style, theme_design, extra_services, other_preferences) VALUES ('$id', '$newid', '$venue', '$guests', '$cuisine', '$style', '$theme', '$services', '$message')";
                    $result2 = mysqli_query($connection, $sql_booking2);

                if($result && $result2)
                {
                    $sql_billing = "INSERT INTO billing (invoice_id, billing_id, booking_id, user_id, amount) VALUES ('$invoice_id', '$bill_id', '$newid', '$id', '$finalPrice') ";
                    ...
                    window.location.href='corporatebook.php';
                    </script>
                    <?php
                } else {
...
<head>
    ...
    <title>Book a Corporate Event | Glamour</title>
...
<body>
        ...
        <ul>
            ...
            <li>
                ...
                <ul class="dropdown">
                    ...
                    <li><a href="corporate-logged.php" class="active" style="font-size: .7em">Corporate</a></li>
                </ul>
            </li>
    ...
    <div class="booking-form" id="booking-form">
        <div class="col1">
            ...
            <div class="bookpic">
                <img src="CSS/Pictures/corporate/corporatebookdisplay.png" style="width: 100%;">
            </div>
        </div>
        <div class="col2">
            <h2>Venue</h2>
            ...
            onclick="return confirm('Please review your information carefully before submitting the form. Are you sure you want to proceed?')">BOOK NOW</button>
        </div>
    </form>
